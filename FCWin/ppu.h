...
public:
    ...
    int get_basentable(){ //使用哪一个nametable
        return data & 3;
    }
    ...
    // 使用哪个图案表的标志位。当标志位为1时，使用0x1000-0x1fff的图案表，否则用0x0000-0x0fff的
    bool get_sptable_dx(){ 
        if (data & (1 << 3)) return 1;
        else return 0;
    }
    //背景使用哪个图案表的标志位。当标志位为1时，使用0x1000-0x1fff的图案表，否则用0x0000-0x0fff的
    bool get_bptable_dx(){ 
...
public:
    ...
    // 获取是否展示最左边八个像素的Background的标志位
    bool get_showedgebkg(){ 
        if (data & (1 << 1)) return 1;
        else return 0;
    }
    // 获取是否展示最左边八个像素的Sprites的标志位
